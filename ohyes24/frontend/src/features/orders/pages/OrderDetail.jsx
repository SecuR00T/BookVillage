...
const TEXT = {
  ...
  unitPrice: "단가",
  lineTotal: "합계",
  shippingHeader: "배송 정보",
  shippingStatus: "배송 상태",
  shippingEta: "도착 예정일",
  shippingAddress: "배송지",
  shippingNumber: "배송 번호",
  shippingGuide: "배송 정보는 주문 상태에 따라 자동 갱신됩니다.",
  shippingNotAvailable: "취소/반품/교환 요청 상태에서는 배송이 진행되지 않습니다.",
  shippingPreparing: "상품 준비 중입니다. 결제 확인 후 출고됩니다.",
  shippingInTransit: "상품이 배송 중입니다. 곧 도착 예정입니다.",
  shippingDone: "배송이 완료되었습니다.",
  etaUnknown: "확인 중",
...
const getShippingInfo = (order) => {
  ...
  const shippingNumber = order?.orderNumber ? `BV-${String(order.orderNumber).replace(/[^A-Z0-9-]/gi, "")}` : "-";

  if (status === "DELIVERED") {
    return {
      statusLabel: "배송 완료",
      eta: formatDateOnly(orderDate),
      guide: TEXT.shippingDone,
      shippingNumber,
    };
  }
  ...
  if (status === "SHIPPED") {
    return {
      statusLabel: "배송 중",
      eta: formatDateOnly(addDays(orderDate, 1)),
      guide: TEXT.shippingInTransit,
      shippingNumber,
    };
  }
  ...
  if (status === "PAID" || status === "PENDING") {
    return {
      statusLabel: "배송 준비중",
      eta: formatDateOnly(addDays(orderDate, 2)),
      guide: TEXT.shippingPreparing,
      shippingNumber,
    };
  }
  ...
  return {
    ...
    eta: TEXT.etaUnknown,
    guide: TEXT.shippingNotAvailable,
    shippingNumber,
  };
};
...
export default function OrderDetail() {
  ...
  const shippingInfo = useMemo(() => getShippingInfo(order), [order]);
    ...
    <PageLayout title={TEXT.title} description={TEXT.description}>
      ...
      {!loading && !error && order && (
        <div className="space-y-4">
          ...
          <section className="rounded-2xl border border-border bg-card p-4">
            <h2 className="mb-3 text-lg font-bold">{TEXT.shippingHeader}</h2>
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-xl border border-border bg-background px-3 py-2">
                <p className="text-xs text-muted-foreground">{TEXT.shippingStatus}</p>
                <p className="mt-1 text-sm font-semibold">{shippingInfo.statusLabel}</p>
              </div>
              <div className="rounded-xl border border-border bg-background px-3 py-2">
                <p className="text-xs text-muted-foreground">{TEXT.shippingEta}</p>
                <p className="mt-1 text-sm font-semibold">{shippingInfo.eta || TEXT.etaUnknown}</p>
              </div>
              <div className="rounded-xl border border-border bg-background px-3 py-2">
                <p className="text-xs text-muted-foreground">{TEXT.shippingAddress}</p>
                <p className="mt-1 text-sm font-semibold">{order.shippingAddress || "-"}</p>
              </div>
              <div className="rounded-xl border border-border bg-background px-3 py-2">
                <p className="text-xs text-muted-foreground">{TEXT.shippingNumber}</p>
                <p className="mt-1 text-sm font-semibold">{shippingInfo.shippingNumber || "-"}</p>
              </div>
            </div>
            <p className="mt-3 text-xs text-muted-foreground">
              {shippingInfo.guide || TEXT.shippingGuide}
            </p>
          </section>
        </div>
      )}
